...
const NavBar: React.FC<{ theme?: "dark" | "light" }> = ({
  ...
  return (
    <nav className={`${userIsScrolled && !menuIsOpen ? 'bg-action-purple' : `${theme === 'light' ? 'bg-white' : 'bg-black'}`} w-full layout-grid col-all transition-all duration-300 z-20 fixed pl-8 pr-8`}>
      <div className={`${userIsScrolled && !menuIsOpen ? 'pt-6 pb-6' : 'pt-16 pb-16'} transition-all duration-300 flex items-center justify-between col-start-2 col-end-8`}>
        <Link href="/">
