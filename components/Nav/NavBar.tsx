  ...
  return (
    <nav className={`${userIsScrolled && !menuIsOpen ? 'bg-action-purple' : `${theme === 'light' ? 'bg-white' : 'bg-black'}`} w-full layout-grid col-all transition-all duration-300 z-20 fixed px-8`}>
      <div className={`${userIsScrolled && !menuIsOpen ? 'pt-4 pb-4' : 'pt-16 pb-16'} transition-all duration-300 flex items-center justify-between col-start-2 col-end-8`}>
        <Link href="/">
          <a className="z-30">{theme === "dark" || userIsScrolled ? <Logo kind="light" /> : <Logo kind="dark" />}</a>
        </Link>
        ...
        <div className={`${menuIsOpen ? 'fixed w-full h-full animate-blur-in' : 'hidden'} inset-0 bg-action-purple z-20 overflow-y-auto flex flex-col`}>
