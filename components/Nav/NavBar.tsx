...
const NavBar: React.FC<{ theme?: "dark" | "light" }> = ({
  ...
  return (
    <nav className={`${userIsScrolled && !menuIsOpen ? 'bg-action-purple' : `${theme === 'light' ? 'bg-white' : 'bg-black'}`} w-full layout-grid col-all transition-all duration-300 z-20 fixed px-8`}>
      <div className={`${userIsScrolled && !menuIsOpen ? 'pt-4 pb-4' : 'pt-16 pb-16'} transition-all duration-300 flex items-center justify-between col-start-2 col-end-8`}>
        <Link href="/">
          <a className="z-30">{theme === "dark" || userIsScrolled || menuIsOpen ? <Logo kind="light" /> : <Logo kind="dark" />}</a>
        </Link>
        ...
        <div className="hidden lg:inline-flex z-30" onClick={toggleMenu}>
          <Lottie
            options={{ animationData: !userIsScrolled && theme === 'light' && !menuIsOpen ? menuAnimationBlack : menuAnimationWhite, autoplay: false, loop: false }}
            height={30}
            width={30}
        ...
        <div className={`${menuIsOpen ? 'fixed w-full h-screen overflow-hidden animate-blur-in' : 'hidden'} inset-0 bg-action-purple z-20 flex flex-col`}>
          ...
          <section className="bg-white p-8">
            <p className="font-header-mono font-thin text-sm text-left uppercase pt-0 pb-6">Follow our stories on:</p>
            <div className="flex">
              <a href="https://github.com/rareagency/rareagency.fi" target="_blank" rel="noreferrer">
                <img
              ...
              <a href="https://www.instagram.com/rarekoodaa" target="_blank" rel="noreferrer">
                <img
              ...
              <a href="https://fi.linkedin.com/company/rareagency" target="_blank" rel="noreferrer">
                <img
